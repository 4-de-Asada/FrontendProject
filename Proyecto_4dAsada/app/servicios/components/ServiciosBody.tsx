...
interface ServiciosBodyProps {
  products: ProductCardProps[];
  categorias: string[];
}

export default function ServiciosBody({ products, categorias }: ServiciosBodyProps) {
  ...
  const visibles =
    categoria === "Todos"
      ? products
      : products.filter((s) => s.category === categoria);
    ...
    <section className={styles.body}>
      ...
      {visibles.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-20 text-gray-400">
          ...
          <p className="text-xs mt-1">Aún no se han publicado servicios en esta categoría.</p>
        </div>
      ) : (
